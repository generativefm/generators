    ...
    ({ samples }) => {
      ...
      return Promise.all(
        [
          ...
          getCustomSampler(samples[instrumentName][preferredFormat], filter)
        )
        ...
        const playHarp = () => {
          const note =
            HARP_NOTES[Math.floor(Math.random() * HARP_NOTES.length)];
        ...
        return () => {
          [harmonics, marimba, harp, piano, filter].forEach(node =>
            node.dispose()
          );
        };
      });
